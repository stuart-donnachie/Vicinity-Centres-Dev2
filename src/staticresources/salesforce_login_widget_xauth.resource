	...
	var XAuthApi = {
		...
		**/
		...
		'xauth::extend': function(originHostname, requestObj) {
			...
			requestObj.token = String(requestObj.token).substr(0,8192); // We cast to String for tokens that are 1's or 0's
			...
			var key = requestObj.communityURL || originHostname;
		...
		**/
		...
		'xauth::expire': function(originHostname, requestObj) {
			...
			storage.removeItem(requestObj.communityURL);
